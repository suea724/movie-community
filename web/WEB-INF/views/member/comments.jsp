                    ...
                    <table class="table">
                        ...
                        <c:forEach var="dto" items="${list}">
                            <tr>
                                ...
                                <td><a href="">${dto.title}<span class="comment"> ${dto.content}</span></a></td>
                                </c:if>
                                <c:if test="${dto.type == '그룹'}">
                                <td>${dto.title}<span class="comment"> ${dto.content}</span></td>
                                </c:if>
                                <td>${dto.regdate}</td>
                            </tr>
                        </c:forEach>
                ...
                <nav aria-label="Page navigation example">
                    <ul class="pagination">
                        ...
                        <c:forEach var="index" begin="${pagination.beginPage}" end="${pagination.endPage}" step="1">
                            <li class="page-item"><a class="page-link" href="/movie/member/mypage/comments.do?page=${index}">${index}</a></li>
                        </c:forEach>
                        <c:if test="${pagination.prev == true}">
...
<script>
    $('#mycomments').css('color', 'rgb(69, 149 , 140)');
</script>
