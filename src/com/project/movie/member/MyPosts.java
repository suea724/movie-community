...
import com.project.movie.dto.MyPostsDTO;
...
@WebServlet("/member/mypage/posts.do")
public class MyPosts extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
        ...
        Pagination pagination = new Pagination(page, dao.getTotalCount(dto.getId()), 20, 10);
        ArrayList<MyPostsDTO> list = dao.getMyPosts(dto.getId(), page);
        setList(list);

        req.setAttribute("pagination", pagination);
        req.setAttribute("list", list);

        RequestDispatcher dispatcher = req.getRequestDispatcher("/WEB-INF/views/member/posts.jsp");
        dispatcher.forward(req, resp);
    }

    private ArrayList<MyPostsDTO> setList(ArrayList<MyPostsDTO> list) {

        LocalDate today = LocalDate.now();

        for (MyPostsDTO dto : list) {
